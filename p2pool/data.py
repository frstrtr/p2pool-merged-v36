...
# ---- G1-HARDEN-1 helper ----------------------------------------------------
# Witness commitment for the L1 tx-set artifact cache key.  txs_key covers
# only (txid, fee), but the cached artifacts embed segwit_data derived from
# the actual witness bytes in known_txs -- an identical (txid, fee) set with
# different witnesses must MISS, not serve a stale witness commitment (lost
# block, not a fork).  Legacy txs are fully committed by their txid already,
# so only segwit txs contribute their wtxid.  Per-tx wtxids are memoized on
# tx object identity (tx dicts are never mutated in place), so steady-state
# cost per template build is one dict lookup + one 'is' check per candidate
# tx -- same order as the txs_key tuple build the caller already pays;
# pack+hash256 runs once per new tx object.  The memo is pruned to the
# current candidate set every call, so it stays bounded by the template size.
_g1_wtxid_memo = {}  # tx_hash -> (tx_object, wtxid)

def _g1_witness_key(desired_other_transaction_hashes_and_fees, known_txs):
    if known_txs is None:
        return None
    fresh = {}
    wtxids = []
    for tx_hash, _fee in desired_other_transaction_hashes_and_fees:
        tx = known_txs.get(tx_hash)
        if tx is None:
            continue
        ent = _g1_wtxid_memo.get(tx_hash)
        if ent is not None and ent[0] is tx:
            fresh[tx_hash] = ent
            wtxids.append(ent[1])
        elif bitcoin_data.is_segwit_tx(tx):
            ent = (tx, bitcoin_data.get_wtxid(tx, None, None))
            fresh[tx_hash] = ent
            wtxids.append(ent[1])
        # else: legacy tx -- its txid (already in txs_key) commits to all
        # of its bytes
    _g1_wtxid_memo.clear()
    _g1_wtxid_memo.update(fresh)
    return frozenset(wtxids)
...
class BaseShare(object):
    ...
    def get_ref_hash(cls, net, share_info, ref_merkle_link, message_data=None):
        ...
        import sys
        coinbase = share_info['share_data'].get('coinbase', b'')
        # Always dump for c2pool shares (90-byte coinbase with /c2pool/ tag)
    ...
    def _get_txset_artifacts(cls, tracker, previous_share_hash, height, desired_other_transaction_hashes_and_fees, net, known_txs, new_tx_budget=None):
        ...
        key = (cls, net, previous_share_hash if cls.VERSION < 34 else None, txs_key,
               _g1_witness_key(desired_other_transaction_hashes_and_fees, known_txs))  # G1-HARDEN-1: bind L1 key to actual witness bytes
...
def get_expected_payouts(tracker, best_share_hash, block_target, subsidy, net):
    ...
    if best_share.VERSION >= 36:
        ...
        for script, weight in weights.iteritems():
            # Try multiple address types: P2PKH, P2SH, then bech32 (P2WPKH/P2WSH)
            addr = None
            try:
                addr = bitcoin_data.script2_to_address(script, net.PARENT.ADDRESS_VERSION, -1, net.PARENT)
            except Exception:
                pass
            if addr is None:
                try:
                    addr = bitcoin_data.script2_to_address(script, net.PARENT.ADDRESS_P2SH_VERSION, -1, net.PARENT)
                except Exception:
                    pass
            if addr is None:
                try:
                    addr = bitcoin_data.script2_to_address(script, -1, 0, net.PARENT)
                except Exception:
                    addr = 'script:' + script.encode('hex')
